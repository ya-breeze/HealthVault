...
- **At and above the breakpoint**, the header SHALL carry app name, active user, webhook link, custom foods link, import link, settings link, and logout — its full set, unchanged.
- **Below the breakpoint**, the header SHALL carry only the app name and the active user. The remaining controls SHALL be reachable through the mobile navigation shell instead, which is responsible for leaving none of them stranded. The header SHALL NOT wrap to additional rows at any viewport width it renders at.
