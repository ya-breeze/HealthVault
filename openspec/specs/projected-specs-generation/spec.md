# projected-specs-generation Specification

## Purpose
TBD - created by archiving change add-projected-specs-artifact. Update Purpose after archive.
## Requirements
...
For each touched capability, the generator SHALL write `openspec/specs.projected/<capability>/spec.md` containing the full post-archive requirement text, and `openspec/specs.projected/<capability>/spec.diff` containing a unified diff (`diff -u`) between the current `openspec/specs/<capability>/spec.md` at committed `HEAD` (or an empty input if the capability does not yet exist at `HEAD`) and that post-archive text. This exists because `openspec/specs.projected/` never persists on `main` (see "Projected specs never persist past merge" below), so there is no git-committed baseline for git's own diff view to compare a PR's projection against; the `.diff` file provides the old-to-new comparison independently of git history. The "before" side SHALL be read from committed `HEAD` (e.g. via `git show HEAD:openspec/specs/<capability>/spec.md`), not from the primary working tree's copy of that file, so an unrelated uncommitted local edit cannot change `spec.diff`'s output.
...
- **THEN** `openspec/specs.projected/widget-api/spec.diff` shows the committed-`HEAD` `openspec/specs/widget-api/spec.md` as the diff's "before" side and the post-archive text as the "after" side
...
#### Scenario: Uncommitted edits to the primary working tree do not affect the diff baseline
- **WHEN** the primary working tree has an uncommitted edit to `openspec/specs/<capability>/spec.md` that differs from committed `HEAD`
- **THEN** re-running the generator without committing that edit produces byte-identical `spec.md` and `spec.diff` output to a run before the edit was made
...
#### Scenario: A bullet-form REMOVED entry conflicts with an operation on the same requirement
- **WHEN** one touched change's delta contains `## REMOVED Requirements` with the bullet form `- `### Requirement: A`` (not a `### Requirement: A` header), and another touched change's delta contains an `ADDED`, `MODIFIED`, or `REMOVED` operation on requirement `A` in the same capability
- **THEN** the generator exits non-zero before creating a worktree, and its output names both conflicting change ids and requirement `A`
