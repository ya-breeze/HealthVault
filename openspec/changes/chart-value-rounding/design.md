...
Precision per type, carried over unchanged from the existing `vitals.ts` switch: 1 decimal for `weight`, `distance`, `sleep` (hours); 0 decimals (whole number) for `steps`, `heart_rate`, `heart_rate_variability`, `oxygen_saturation`, and the rest of the `point`-family vitals not already covered by `vitals.ts` get 0 decimals by default (matches how e.g. `blood_glucose`, `resting_heart_rate` etc. are whole-number-ish metrics with no existing convention to contradict) — **flagged as an open question below** for any metric where 0-decimals isn't obviously right.
