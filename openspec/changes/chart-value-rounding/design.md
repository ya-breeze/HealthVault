...
- **Cumulative-family types** not in `vitals.ts`: `active_calories`, `total_calories` (kcal, whole numbers), `exercise` (`DurationSeconds` rendered as minutes, whole number), `nutrition` (grams/calories — matches the existing `Math.round`/`.toFixed(0)` convention already used by `MacroSummary.tsx`/food history). `hydration` (`Liters`) is the one exception in this group — 1 decimal, not 0, since sub-liter precision (e.g. `1.5`) is meaningful and matches `distance`'s treatment as a liquid/distance-style quantity.
