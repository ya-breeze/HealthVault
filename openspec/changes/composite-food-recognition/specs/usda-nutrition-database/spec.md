...
When a custom food does not match by exact name, the system SHALL additionally retrieve the user's own custom foods ranked by a frequency-and-recency score computed from that user's prior meal history (frequency weighted higher than recency, so a dish eaten only monthly still ranks above a food used once recently), and SHALL include the top-ranked of these in the candidate shortlist offered for selection alongside whatever Open Food Facts or USDA candidates the rest of this requirement would otherwise offer. Selection SHALL be instructed to prefer a candidate from the user's own custom foods when it is a reasonable match for the recognized item, since it reflects previously verified real-world macros rather than a generic reference profile. This frequency/recency-ranked retrieval does not change how Open Food Facts or USDA are queried or routed between each other; it only adds custom-food candidates to the shortlist selection considers.
...
- **WHEN** no custom food exactly matches a recognized item's name, and the user has a custom food that has been used repeatedly (or at least once a month) across their meal history
