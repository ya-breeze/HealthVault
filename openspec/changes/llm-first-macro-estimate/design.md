...
- Declared `calories_per_100g` is not lower than the Atwater estimate `protein*4 + carbs*4 + fat*9` by more than a small tolerance (e.g. the larger of 15% or 25 kcal). This check is intentionally one-sided: declared calories exceeding the macro-derived figure is normal and expected (alcohol contributes ~7 kcal/g and isn't tracked as a macro here, and there's ordinary rounding slop), so an estimate is never rejected for having *more* calories than its P/C/F imply — only for having *fewer*, which is the internally-inconsistent direction (protein/carbs/fat are each non-negative calorie contributions, so declared calories can't genuinely fall short of what they alone add up to). A two-sided tolerance was considered and rejected: it flagged correct estimates for wine, beer, and alcohol-reduction sauces as implausible purely because their alcohol calories aren't captured by P/C/F, which would have defeated the change's goal for that whole food category.
