...
An estimated profile is **usable** when its values pass validation (a valid, non-negative estimate, as before) AND pass a plausibility check: no per-100g macro (protein, carbohydrates, fat) exceeds 100g, sugar and dietary fiber each do not exceed carbohydrates by more than a small tolerance, and declared calories are not lower than the Atwater calculation from protein/carbohydrates/fat (protein×4 + carbs×4 + fat×9) by more than a small tolerance. This calorie check is one-sided: declared calories exceeding the Atwater figure is not a violation, since sources such as alcohol contribute calories the three tracked macros don't capture. An estimate that is present but fails this plausibility check SHALL be treated as absent for precedence purposes, the same as if Recognize had produced none.
...
#### Scenario: A deterministic identity match still takes precedence over the estimate
...
- **WHEN** Recognize produces an estimated profile for a recognized item that is valid (non-negative) but fails the plausibility check (e.g. a per-100g macro exceeding 100g, or declared calories wildly inconsistent with its own protein/carbs/fat), and candidate selection selects a suitable USDA, Open Food Facts, or custom-food candidate for that item
