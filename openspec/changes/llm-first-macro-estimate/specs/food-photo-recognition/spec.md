...
An estimated profile is **usable** when its values pass validation (a valid, non-negative estimate, as before) AND pass a plausibility check: no per-100g macro (protein, carbohydrates, fat) exceeds 100g, sugar and dietary fiber each do not exceed carbohydrates by more than a small tolerance, and declared calories are within a wide tolerance of the Atwater calculation from protein/carbohydrates/fat (protein×4 + carbs×4 + fat×9). An estimate that is present but fails this plausibility check SHALL be treated as absent for precedence purposes, the same as if Recognize had produced none.
