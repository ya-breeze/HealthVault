...
- **THEN** the backend container mounts that directory at `/data`, and all backend env vars that reference on-disk paths (`HCW_DBPATH`, `HCW_UPLOADS_DIR`, `HCW_USDA_DB_PATH`, `HCW_OFF_DB_PATH`) resolve inside it
