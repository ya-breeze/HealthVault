...
- Replace the data page's single always-raw line chart with a Day / Week / Month / Year segmented control. Week and Month both request `day`-granularity aggregation and differ only in time range; Year requests `month`-granularity — there is no week-sized bucket. Each level selects a time range and, for Week/Month/Year, an aggregation:
  - **Cumulative types** (`steps`, `distance`, `active_calories`, `total_calories`, `hydration`, `exercise`, `nutrition`, `sleep` by night): Day = raw points as a line; Week/Month = per-day (or per-night) sum as bars; Year = per-month sum as bars. `nutrition`'s seven macro columns are summed independently.
  - **Point-in-time types** (`heart_rate`, `heart_rate_variability`, `weight`, `height`, `blood_pressure`, `blood_glucose`, `oxygen_saturation`, `body_temperature`, `skin_temperature`, `respiratory_rate`, `resting_heart_rate`, `vo2_max`, `body_fat`, `lean_body_mass`, `bone_mass`, `speed`, `basal_metabolic_rate`): Day = raw points as a line; Week/Month/Year = a bucketed average line with a shaded min–max band per bucket.
- `GET /api/data/{type}` gains an optional `?bucket=` parameter (`day` or `month`) that returns pre-aggregated rows (`bucket_start`, `avg`/`sum`, `min`, `max`, `count`) instead of raw records, so Month/Year views don't require transferring hundreds of thousands of raw rows to the browser. Omitting `?bucket=` keeps today's raw-record response unchanged — existing callers (E2E tests, any other consumer) are unaffected.
...
- **Frontend:** `app/layout.tsx` (shared header extracted into a component used app-wide), `app/page.tsx` (dashboard rewrite), `app/data/[type]/DataTypeClient.tsx` (zoom control + aggregation-aware chart rendering), `app/globals.css` (design tokens), new components for the vitals card, zoom chart, and icon set, `lib/api.ts` (new `bucket` param on `api.data()`). Also touches the four pages that currently render their own copy of the header markup instead of a shared component: `app/food/upload/page.tsx`, `app/food/manual/page.tsx`, `app/food/review/ReviewClient.tsx`, `app/food/custom/page.tsx` (plus `app/import/page.tsx`, already covered above).
- **Backend:** `backend/pkg/server/api.go` (`dataHandler` accepts `?bucket=`), `backend/pkg/database/storage.go` / `storage_impl.go` (new aggregation query alongside existing `QueryRecords`), one new query per cumulative (`SUM`) vs. point-in-time (`AVG`/`MIN`/`MAX`) type family, bucketed by truncating the type's time column to day/month in SQLite.
