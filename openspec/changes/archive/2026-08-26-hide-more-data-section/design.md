...
- **Settings-load gating is a new dependency for a section that never had one** — mitigated by reusing the exact same `dashboardReady` flag Phase 1 already built for the vitals grid, not inventing a second one. Unlike the vitals grid, More Data gets no dedicated loading/error placeholder of its own: it continues to silently render nothing while not ready (today it does this for `presenceReady`; this change extends the same silent condition to `dashboardReady`). A settings-load failure therefore leaves the section invisible until the user retries via the vitals grid's existing retry control — a small new failure mode (today a settings-load failure has no effect on this section at all, since it never depended on settings), accepted because building a second dedicated error UI for one boolean is disproportionate.
