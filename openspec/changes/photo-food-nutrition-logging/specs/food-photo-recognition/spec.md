...
The system SHALL reject an upload before storing it when the request body exceeds the configured maximum size (`HCW_MAX_UPLOAD_BYTES`, default 10 MiB) or when the decoded content is not a JPEG, PNG, HEIC, or WebP image. Image type SHALL be determined by sniffing the content, not by the declared `Content-Type`. The stored file path SHALL be generated entirely by the server and SHALL NOT incorporate any client-supplied filename.
...
The system SHALL expose `POST /api/food/meals/{id}/retry`, which re-runs vision analysis against the already-stored photo for a meal owned by the caller whose status is `failed` or `processing`. Because analysis runs synchronously within the upload request, a meal left in `processing` is a crash remnant and SHALL be recoverable through this same endpoint without any background reconciliation job.
...
- **WHEN** the server restarts while a meal is in `processing` and the owner later calls retry on it
- **THEN** the system re-runs analysis on the stored photo and the meal leaves `processing`
