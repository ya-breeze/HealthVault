...
**Brand is a required filter, not a ranking term — this is not optional, it's what makes the gate mean anything.** `off.Search`'s signature takes `name` and `brand` as separate parameters, not one opaque OR-joined string. The query built from them requires the `brands` FTS column to match the brand terms (`brands:olma`, an AND-scoped predicate) while `product_name` terms are used for ranking within that brand-filtered set (OR'd among themselves, as `usda.QueryFor` already does for name/prep/state — that OR-recall behavior is still correct for *ranking*, just not for the brand predicate). Naively reusing `usda.QueryFor`'s pattern — OR-joining every token including brand ones into one flat query — was considered and is explicitly wrong here: "Olma jogurt" would become `"olma" OR "jogurt"`, so *any* yogurt product matches regardless of brand, and the whole brand-gate collapses back into the exact "arbitrary branded product" risk it exists to prevent. Every returned OFF candidate under this design has already matched the extracted brand by construction — the brand-presence gate and the brand-match-required query are two halves of the same requirement, not independent choices.
