...
- [x] Create `backend/pkg/server/food_advice.go` with the request and response types from `How`, and register `api.HandleFunc("/food/advice", fh.PostFoodAdvice).Methods("POST")` in `backend/pkg/server/server.go` beside the other `/food/*` routes
- [x] Require `isSameOriginRequest` before any cache access or model call, answering 403 for a browser `Sec-Fetch-Site` other than `same-origin`; preserve the helper's missing-header allowance for non-browser clients and tests
