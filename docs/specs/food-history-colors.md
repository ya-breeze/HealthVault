...
- Add three theme tokens, `--outcome-counted` (green), `--outcome-attention` (amber) and
  `--outcome-empty` (grey), with a light and a dark value, next to the existing `--c-*` tokens.
...
- [x] Add a unit test that each outcome maps to a distinct color class
- [ ] Verify in a browser against WIP, light and dark
- [ ] Mark completed
