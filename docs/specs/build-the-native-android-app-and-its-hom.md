...
- [ ] Replace the undersized adaptive launcher icon in 1x1, 2x1, 1x2, and reduced layouts with legible text identity, including stale-state treatment
- [ ] Recompose 2x2 around a centered calorie group without a full-height weighted gap, while preserving missing-target, over-target, large-font, and whole-card tap behavior
- [ ] Add a dedicated FlexWindow Glance widget, receiver, picker preview, `keyguard` provider metadata, and Samsung `sub_screen` metadata using the documented full-screen bounds
- [ ] Keep update-all, placement counting, periodic refresh, sign-out, and failure states correct when either or both widget providers are placed
- [ ] Add red-capable regression tests for the white-dot source, 2x2 empty-gap source, FlexWindow registration/layout, and dual-provider refresh lifecycle
