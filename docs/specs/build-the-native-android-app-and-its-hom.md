...
- [ ] Make 2x1 the default size, support a useful optional 1x1 minimum, resize on both axes through 4x2, and cap the provider height at two rows
- [ ] Open **Log food** from the loaded or stale card surface, keep signed-out and error taps routed to the native app, and show separate add and refresh controls only in the full 4x2 layout
- [ ] Anchor the calorie group to the bottom of 2x2, and replace the compressed 4x2 macro strip with three full-width rows distributed over the available height
- [ ] Keep missing-target, over-target, stale-state, localization, Material color, touch-target, and picker-preview behavior correct at every breakpoint
- [ ] Add regression coverage for every declared breakpoint, tap routing, provider bounds, and height-aware composition
