...
**The widget** is a single Glance `AppWidgetProvider` with `SizeMode.Responsive`, not two separate widgets: the owner places one widget and resizes it, rather than choosing a size in the picker and having to delete and re-add to change it. Both layouts treat today's calories as the visual hero instead of compressing the web card. The compact layout (about 110x110dp, a 2x2 cell) uses the available surface for a HealthVault label, a large consumed-calorie number, its target and percentage, and a full-width calorie progress indicator. The wide layout (about 250x110dp, a 4x2 cell) keeps that hierarchy, adds three macro rows with inline labels and flexible `LinearProgressIndicator` bars, and places **Log food** and refresh in a horizontal pair of 48dp targets at the upper right so they fit the 110dp height. When no target is available, both layouts show the consumed value without a fabricated percentage or progress denominator; progress bars clamp consumption above target while the visible percentage may exceed 100%. Material widget color roles, a resource-backed system widget radius with a pre-Android 12 fallback, and flexible widths replace fixed black/green styling and fixed 90dp bars. Glance 1.1 has no generated-preview API, so the picker receives a hand-written XML preview representative of the compact layout plus a concise description. The three macro bars correct the original Task 5 wording: the API and implementation expose protein, carbohydrates, and fat, while calories remain the separate hero value. The originally planned recommendation slot was cut during integration: newer work on `main` retired `summaryTodayResponse.recommendation` and moved nutrition advice to `POST /api/food/advice`, whose client-computed input and model latency do not belong in this cheap summary read.
...
- [ ] Make the compact layout fill its surface with the HealthVault label, a large consumed-calorie value, target and percentage, and a full-width `LinearProgressIndicator`
- [ ] Make the wide layout use flexible `LinearProgressIndicator` bars and inline labels for the three macros, with a horizontal pair of 48dp primary **Log food** and secondary refresh actions
- [ ] Preserve unavailable-target semantics, clamp progress above target, and keep the visible percentage truthful
- [ ] Use Material widget color roles, a system-radius resource with an API 26–30 fallback, and localized accessible action descriptions in both light and dark themes
- [ ] Add a representative hand-written XML picker preview and description, pure progress-model tests, and source/XML regression guards for compact and wide content
- [ ] Build, test, lint, and assemble the Android app
- [ ] Mark completed
