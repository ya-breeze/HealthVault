...
// package shipped before saturated_fat existed, to prove Open/Search/ByCode
...
func buildPreSaturatedFatIndex(t *testing.T, code, name, brands string, kcal float64) string {
	t.Helper()
...
`
	...
	if _, err := db.Exec(
		`INSERT INTO off_foods (code, product_name, brands, calories, protein) VALUES (?,?,?,?,?)`,
		code, name, brands, kcal, 5.0,
	); err != nil {
		t.Fatalf("insert legacy row: %v", err)
	}
	if _, err := db.Exec(`INSERT INTO off_foods_fts(off_foods_fts) VALUES('rebuild')`); err != nil {
...
func TestOpen_PreSaturatedFatDatabaseDegradesGracefully(t *testing.T) {
	path := buildPreSaturatedFatIndex(t, "111", "Bílý jogurt", "Olma", 65)
	...
	byCode, err := idx.ByCode("111")
	if err != nil {
		t.Fatalf("ByCode on a pre-upgrade database must not fail: %v", err)
	}
	if byCode == nil || byCode.Profile.CaloriesPer100g != 65 || byCode.Profile.SaturatedFatPer100g != 0 {
		t.Fatalf("got %+v, want the yogurt profile with SaturatedFatPer100g 0", byCode)
	}

	results, err := idx.Search("jogurt", "Olma", off.DefaultCandidates)
	if err != nil {
